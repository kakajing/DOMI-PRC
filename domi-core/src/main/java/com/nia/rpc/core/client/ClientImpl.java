...
public class ClientImpl implements Client{
    ...
    private static CopyOnWriteArrayList<ChannelConf> channelWrappers = new CopyOnWriteArrayList<>();
    ...
    public Response sendMessage(Class<?> clazz, Method method, Object[] args) {
        ...
        //当channel的配置链接不为空的时候，就可以取到chanel了
        Channel channel = null;
        try {
            channelWrapper.getChannelObjectPool().borrowObject();
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try {
            ...
            return blockingQueue.poll(requestTimeoutMillis, TimeUnit.MICROSECONDS);
